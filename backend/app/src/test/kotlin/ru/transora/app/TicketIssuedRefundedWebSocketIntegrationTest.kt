...
class TicketIssuedRefundedWebSocketIntegrationTest : IntegrationTestBase() {
    @Autowired
    ...
    @Test
    fun `station agent receives ticket issued event after sale`() {
        ...
        val sale = createTripShiftAndTicket()
        waitForOutbox()

        val ticketIssued = pollMessage(messages, 15, TimeUnit.SECONDS) {
            it.contains("\"type\":\"ticket.issued\"") && it.contains("\"ticketId\":\"${sale.ticketId}\"")
        }
        assertThat(ticketIssued).isNotNull
        assertThat(ticketIssued).contains("\"status\":\"ISSUED\"")
        assertThat(ticketIssued).contains("\"passengerName\":\"WS Issued Passenger\"")

        session.close()
    }

    @Test
    fun `station agent receives ticket refunded event after refund`() {
        val messages = ConcurrentLinkedQueue<String>()
        val connected = CountDownLatch(1)
        val handler = object : TextWebSocketHandler() {
            override fun afterConnectionEstablished(session: WebSocketSession) {
        ...
        val sale = createTripShiftAndTicket()
        waitForOutbox()
        pollMessage(messages, 15, TimeUnit.SECONDS) {
            it.contains("\"type\":\"ticket.issued\"")
        }

        mockMvc.post("/api/tickets/${sale.ticketId}/refund") {
        ...
        val ticketRefunded = pollMessage(messages, 15, TimeUnit.SECONDS) {
            it.contains("\"type\":\"ticket.refunded\"") && it.contains("\"ticketId\":\"${sale.ticketId}\"")
        }
        assertThat(ticketRefunded).isNotNull
        assertThat(ticketRefunded).contains("\"status\":\"REFUNDED\"")

        session.close()
    }

    private data class SaleContext(val tripId: String, val ticketId: String)

    private fun createTripShiftAndTicket(): SaleContext {
        ...
        val shiftJson = mockMvc.post("/api/shifts") {
            header(HttpHeaders.AUTHORIZATION, adminAuth)
            contentType = MediaType.APPLICATION_JSON
            content = """{"stationName":"T1","cashierName":"ws-issued-cashier-$unique","posId":"POS-WS-ISSUED-$unique"}"""
        }.andReturn().response.contentAsString
            ...
            content = """
                {
                  ...
                  "tripId": "$tripId",
                  "seatNumber": 12,
                  "passengerName": "WS Issued Passenger",
                  "docType": "PASSPORT_RF",
        ...
        return SaleContext(tripId = tripId, ticketId = ticketId)
    }
